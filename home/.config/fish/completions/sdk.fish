...
function __get_candidates_versions
    curl --silent "$SDKMAN_CANDIDATES_API/candidates/$argv/$SDKMAN_PLATFORM/versions/all" \
    | string replace -a -r ',' '\n'
end

function __fish_sdkman_versions
    set cmd (commandline -opc)
    set candidate $cmd[3]
    set candidateDir "$SDKMAN_DIR"/candidates/$cmd[3]
    set candidateCache "$SDKMAN_DIR"/candidates/$cmd[3]/cache.txt

    if not [ -f $candidateCache ]
        mkdir -p $candidateDir
        touch $candidateCache
        __get_candidates_versions $candidate > $candidateCache
    end

    set current (stat -c %Y $candidateCache)
    set now (stat -c %Y -)
    
    # 86400 = 1 day (60 * 60 * 24)

    if [ 86400 -lt (math "$now - $current") ]
        __get_candidates_versions $candidate > $candidateCache
    end
